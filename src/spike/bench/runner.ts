 ...
 * `out/spike/bench/launch.js`, drives `layeredkb.spike.runBench`, and writes the
...
export async function run(): Promise<void> {
	...
	const result = await vscode.commands.executeCommand('layeredkb.spike.runBench', request);
	...
	const themes = process.env.LAYEREDKB_SPIKE_THEMES
		? await vscode.commands.executeCommand('layeredkb.spike.probeThemes')
		: undefined;
