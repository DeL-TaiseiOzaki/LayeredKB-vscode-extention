...
> **Authority:** the settled decisions in `.claude/docs/DESIGN.md` (Key Decisions, 2026-09-07
> → 2026-09-09) and `.claude/docs/plans/multi-drive-contents-2026-09.md` are **inputs**, not
...
> 私だけができればいいわけではない。」
> **Companion:** `.claude/docs/research/rclone-verification-procedure.md` — the checks that
...
<https://rclone.org/drive/> (both 2026-09-10). A LayeredKB-shipped `client_id` used for mount
...
<https://support.google.com/cloud/answer/13464323> (2026-09-10). If LayeredKB becomes a company
...
`vscode://del-taiseiozaki.layeredkb/...` redirect is **not** usable with a Desktop client, and
...
this with a Microsoft-hosted redirect; LayeredKB has no such host and should not acquire one.
...
The split is right and is exactly what makes onboarding decidable: `.layeredkb/mounts.yaml` is
...
resolves, files are there, the state machine says `attached`, and LayeredKB indexes and presents
...
1. **Install VS Code and the LayeredKB extension** from the Marketplace.
...
work is a `layeredkb.connectMount` command driven by a QuickPick (mount list) plus
...
Meanwhile the product's own premise argues for staying: LayeredKB exists for "people who run a
...
- *"Claim A holds with caveats — a distributed desktop app can ship a Drive client id at
  ...
  single-maintainer MIT extension. The verdict stays, scoped as Codex phrases it: LayeredKB will
