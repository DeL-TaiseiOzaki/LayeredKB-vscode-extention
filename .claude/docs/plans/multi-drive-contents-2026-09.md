...
> Related: `.claude/docs/DESIGN.md` FR-9..FR-13 and Constraints. Codex consult:
...
exchange input whose internals never participate in LayeredKB at all, no layer classification, no
...
| Store | Tracked? | Job |
|---|---|---|
| `.layeredkb/mounts.yaml` | **tracked**, schema layer | *What should be attached*: `id`, `mountPoint` (vault-relative), `kind`, `scope`, `substrate`, `required`, `obtain` (human instructions), `expectedIdentity`. **No absolute paths, ever.** |
...
- 🟢 **Naming.** This design adds `kind: scope|exchange`, `contents/`, `.layeredkb/mounts.yaml`. The
...
Obsidian plugin lives in this repository; migration of the shipped `layeredkb.layers` default.
