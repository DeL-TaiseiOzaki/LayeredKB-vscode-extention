...
**Binding constraints** (from `.claude/docs/DESIGN.md` and the user's design note):
...
- **Knowledge base: yes, but the wrong half.** `teamwiki/` is git-tracked markdown — precisely
  the `schema` + `Knowledge_Base` side that LayeredKB already handles with git. It has nothing
...
only block LayeredKB from *shipping* macFUSE inside a `.vsix`, which nothing in the design
proposes.
