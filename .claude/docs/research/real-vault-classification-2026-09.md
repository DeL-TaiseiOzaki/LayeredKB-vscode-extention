# Does LayeredKB actually split a real vault? (measured, 2026-09-09)
...
the shipped `layeredkb.exclude` — which of the two removes it was not isolated. Nothing about
...
mount is visible out of the box. Note also that `layeredkb.followSymlinks` was `false`
...
6. **`layeredkb.followSymlinks` does not do what its name suggests** for the primary case: the
