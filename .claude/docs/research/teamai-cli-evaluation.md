...
`.claude/docs/research/drive-mount-and-team-cli-2026-09.md` §1 ("`Tencent/teamai-cli`"), which
...
`schema` + `Knowledge_Base` layers LayeredKB itself exists to manage (source research §1.3). If
this repository (or its developer's other projects) ever gets an actual LayeredKB-managed
...
| R8 | Team-knowledge duplication: `teamwiki/` (teamai-cli) vs. any future LayeredKB-managed knowledge base — two independently maintained "team knowledge" surfaces with no defined relationship, and (unlike R7) no documented resolution order between them. | **Medium** | §3.2, §3.4 analysis; teamai-cli's `teamwiki/` feature (primary source §1.2). | Keep `teamwiki/` and any LayeredKB knowledge base explicitly scoped to different purposes if both exist. |
...
- `.claude/docs/research/drive-mount-and-team-cli-2026-09.md` — primary research; all teamai-cli
